...
void usage_info(void) {
    pizza_io_puts("{cyan,bold}==================================================\n");
    pizza_io_puts("{white,bold}\tUsage: fossil [options] [command]\n");
    pizza_io_puts("{cyan,bold}==================================================\n\n");

    pizza_io_puts("{yellow,bold}Options:\n");
    for (size_t i = 0; i < sizeof(FOSSIL_TEST_OPTIONS) / sizeof(FOSSIL_TEST_OPTIONS[0]); i++) {
        pizza_io_printf("  {green}> %s\n", FOSSIL_TEST_OPTIONS[i]);
    }
...
void version_info(void) {
    pizza_io_puts("{magenta,bold}==================================================\n");
    pizza_io_puts("{white,bold}\tFossil Logic Test Framework\n");
    pizza_io_puts("{magenta,bold}==================================================\n");

    pizza_io_printf("  {cyan}Version: %s\n", FOSSIL_TEST_VERSION);
    pizza_io_printf("  {cyan}Author : %s\n", FOSSIL_TEST_AUTHOR);
    pizza_io_printf("  {cyan}License: %s\n", FOSSIL_TEST_LICENSE);
...
void fossil_test_register_suite(fossil_test_env_t *env, fossil_test_suite_t *suite) {
    ...
    if (env->options.show_info) {
        pizza_io_printf("{blue,bold}Registered test suite: %s\n{reset}", suite->name);
    }
}
...
void fossil_test_run_suite(fossil_test_suite_t *suite, fossil_test_env_t *env) {
    ...
    if (env->options.show_info) {
        switch (env->options.format) {
            case FOSSIL_TEST_FORMAT_CI:
                pizza_io_printf("[SUITE] %s\n", suite->name);
                break;
            case FOSSIL_TEST_FORMAT_JELLYFISH:
                pizza_io_printf("{brightblue}[Suite: %s]{reset}\n", suite->name);
                break;
            case FOSSIL_TEST_FORMAT_PLAIN:
            default:
                pizza_io_printf("{blue,bold}Running suite: %s{reset}\n", suite->name);
                break;
        }
    }
    ...
    if (env->options.show_info) {
        switch (env->options.format) {
            case FOSSIL_TEST_FORMAT_CI:
                pizza_io_printf("[SUITE DONE] %s (%.3fs)\n", suite->name, total_execution_time);
                break;
            case FOSSIL_TEST_FORMAT_JELLYFISH:
                pizza_io_printf("{cyan}⏱ Suite %s completed in %.3f seconds{reset}\n", suite->name, total_execution_time);
                break;
            case FOSSIL_TEST_FORMAT_PLAIN:
            default:
                pizza_io_printf("{cyan}Total execution time for suite %s: %.3f seconds{reset}\n", suite->name, total_execution_time);
                break;
        }
    }
}
...
void fossil_test_run_case(fossil_test_case_t *test_case, fossil_test_env_t *env) {
    ...
    if (env->options.dry_run) {
        pizza_io_printf("{purple}Dry run mode enabled. No tests will be executed.{reset}\n");
        return;
    }
    ...
    switch (test_case->status) {
        case TEST_STATUS_PASS:
            if (env->options.show_info) {
                switch (env->options.format) {
                    case FOSSIL_TEST_FORMAT_PLAIN:
                        pizza_io_printf("{green}PASSED: %s (%.3fs){reset}\n", test_case->name, test_case->execution_time);
                        break;
                    case FOSSIL_TEST_FORMAT_CI:
                        pizza_io_printf("::notice file=%s,line=0::PASSED: %s (%.3fs)\n",
                                             test_case->name, test_case->name, test_case->execution_time);
                        break;
                    case FOSSIL_TEST_FORMAT_JELLYFISH:
                        pizza_io_printf("{green}✔ PASS: {blue}%s {dim}(%.3fs){reset}\n",
                                             test_case->name, test_case->execution_time);
                        break;
                }
            }
        ...
        case TEST_STATUS_FAIL:
            ...
            switch (env->options.format) {
                case FOSSIL_TEST_FORMAT_PLAIN:
                    pizza_io_printf("{red}FAILED: %s{reset}\n", test_case->name);
                    break;
                case FOSSIL_TEST_FORMAT_CI:
                    pizza_io_printf("{red}::error file=%s,line=0::FAILED: %s{reset}\n",
                                         test_case->name, test_case->name);
                    break;
                case FOSSIL_TEST_FORMAT_JELLYFISH:
                    pizza_io_printf("{red}✖ FAIL: {blue}%s{reset}\n", test_case->name);
                    if (test_case->failure_message) {
                        pizza_io_printf("{red}  → Reason: %s{reset}\n", test_case->failure_message);
                    }
                    break;
            }
            break;
        ...
        case TEST_STATUS_SKIP:
            ...
            switch (env->options.format) {
                case FOSSIL_TEST_FORMAT_PLAIN:
                    pizza_io_printf("{yellow}SKIPPED: %s{reset}\n", test_case->name);
                    break;
                case FOSSIL_TEST_FORMAT_CI:
                    pizza_io_printf("{yellow}::warning file=%s,line=0::SKIPPED: %s{reset}\n",
                                         test_case->name, test_case->name);
                    break;
                case FOSSIL_TEST_FORMAT_JELLYFISH:
                    pizza_io_printf("{yellow}⚠ SKIP: {blue}%s{reset}\n", test_case->name);
                    break;
            }
            break;
        ...
        case TEST_STATUS_TTIMEOUT:
            ...
            switch (env->options.format) {
                case FOSSIL_TEST_FORMAT_PLAIN:
                    pizza_io_printf("{orange}TIMEOUT: %s\n{reset}", test_case->name);
                    break;
                case FOSSIL_TEST_FORMAT_CI:
                    pizza_io_printf("{orange}::error file=%s,line=0::TIMEOUT: %s{reset}\n",
                                         test_case->name, test_case->name);
                    break;
                case FOSSIL_TEST_FORMAT_JELLYFISH:
                    pizza_io_printf("{orange}⏳ TIMEOUT: {blue}%s{reset}\n", test_case->name);
                    break;
            }
            break;
        ...
        default:
            ...
            pizza_io_printf("{magenta}Unexpected test result for: %s{reset}\n", test_case->name);
            break;
    }
}
...
void fossil_test_init(fossil_test_env_t *env, int argc, char **argv) {
    ...
    if (env->options.dry_run) {
        pizza_io_printf("{purple}Dry run mode enabled. No tests will be executed or evaluated.{reset}\n");
        return;
    }
}
    ...
    switch (env->options.summary) {
        case FOSSIL_TEST_SUMMARY_PLAIN:
            ...
            if (env->pass_count > 0 && env->fail_count == 0 && env->timeout_count == 0) {
                ...
                pizza_io_printf("{cyan}Comment: The test results are mixed. Consider analyzing individual test cases to uncover underlying issues.{reset}\n");
            }
            break;
        ...
        case FOSSIL_TEST_SUMMARY_CI:
            ...
            if (env->pass_count > 0 && env->fail_count == 0 && env->timeout_count == 0) {
                pizza_io_printf("COMMENT=Success! %s\n", great_news_messages[rand() % (sizeof(great_news_messages) / sizeof(great_news_messages[0]))]);
            } else if (env->fail_count > 0) {
                pizza_io_printf("COMMENT=Oops, there were failures: %s\n", humorous_messages[rand() % (sizeof(humorous_messages) / sizeof(humorous_messages[0]))]);
            } else if (env->timeout_count > 0) {
                pizza_io_printf("COMMENT=Timeout encountered: %s\n", timeout_messages[rand() % (sizeof(timeout_messages) / sizeof(timeout_messages[0]))]);
            } else if (env->pass_count == 0 && env->fail_count == 0 && env->timeout_count == 0) {
                pizza_io_printf("COMMENT=No results! %s\n", sarcastic_messages[rand() % (sizeof(sarcastic_messages) / sizeof(sarcastic_messages[0]))]);
            } else {
                pizza_io_printf("COMMENT=Mixed results: Consider reviewing individual test cases.\n");
            }
            break;
        ...
        case FOSSIL_TEST_SUMMARY_JELLYFISH:
            ...
            if (env->pass_count > 0 && env->fail_count == 0 && env->timeout_count == 0) {
                pizza_io_printf("{blue}{bold}Great News!{reset} {cyan}%s{reset}\n", great_news_messages[rand() % (sizeof(great_news_messages) / sizeof(great_news_messages[0]))]);
            } else if (env->fail_count > 0) {
                pizza_io_printf("{red}{bold}Oops, Something Went Wrong!{reset} {orange}%s{reset}\n", humorous_messages[rand() % (sizeof(humorous_messages) / sizeof(humorous_messages[0]))]);
                pizza_io_printf("{red}Failure detected, please review test logs for more information.{reset}\n");
            } else if (env->timeout_count > 0) {
                pizza_io_printf("{orange}{bold}Timeouts Happened!{reset} {yellow}%s{reset}\n", timeout_messages[rand() % (sizeof(timeout_messages) / sizeof(timeout_messages[0]))]);
                pizza_io_printf("{yellow}It might be worth investigating system performance or external factors.{reset}\n");
            } else if (env->pass_count == 0 && env->fail_count == 0 && env->timeout_count == 0) {
                pizza_io_printf("{cyan}{bold}No Results?{reset} {magenta}%s{reset}\n", sarcastic_messages[rand() % (sizeof(sarcastic_messages) / sizeof(sarcastic_messages[0]))]);
                pizza_io_printf("{magenta}Perhaps there were no tests run or something went awry!{reset}\n");
            } else {
                pizza_io_printf("{blue}{italic}Mixed Results Detected!{reset} {green}A closer look at individual test cases might reveal more details.{reset}\n");
            }
            break;

        default:
            pizza_io_printf("{red}Unknown summary mode.{reset}\n");
            break;
    }
}
...
void fossil_test_analyze(fossil_test_env_t *env) {
    ...
    switch (env->options.summary) {
        case FOSSIL_TEST_SUMMARY_PLAIN:
            ...
            if (env->fail_count > 0) {
                pizza_io_printf("{cyan}Failure rate: %.2f%%{reset}\n", failure_rate);
            }
            if (env->timeout_count > 0) {
                pizza_io_printf("{cyan}Timeout tests: %.2f%%{reset}\n", timeout_rate);
            }
            if (env->skip_count > 0) {
                pizza_io_printf("{cyan}Skipped tests: %.2f%% (%d tests){reset}\n", skip_rate, env->skip_count);
            }
            if (env->pass_count > 0) {
                pizza_io_printf("{cyan}Success rate: %.2f%%{reset}\n", success_rate);
            }

            pizza_io_printf("{cyan}Probability of success: %.2f{reset}\n", probability_of_success);
            pizza_io_printf("{cyan}Average test rate: %.2f%%{reset}\n", average_rate);
            pizza_io_printf("{cyan}Prediction (Future Success Rate): %.2f%%{reset}\n", prediction);
            if (env->skip_count > 0) {
                pizza_io_printf("{yellow}Note: There were %d skipped tests. Please check the conditions or requirements for those tests.{reset}\n", env->skip_count);
            }
            break;
        ...
        case FOSSIL_TEST_SUMMARY_CI:
            ...
            if (env->fail_count > 0) {
                pizza_io_printf("::group::{bold}Test Failure Analysis (CI Mode){reset}\n");
                pizza_io_printf("FAILURE_RATE=%.2f%%\n", failure_rate);
            }
            if (env->timeout_count > 0) {
                pizza_io_printf("::group::{bold}Test Timeout Analysis (CI Mode){reset}\n");
                pizza_io_printf("TIMEOUT_RATE=%.2f%%\n", timeout_rate);
            }
            if (env->skip_count > 0) {
                pizza_io_printf("::group::{bold}Test Skipped Analysis (CI Mode){reset}\n");
                pizza_io_printf("SKIP_RATE=%.2f%%\n", skip_rate);
                pizza_io_printf("SKIPPED_TESTS=%d\n", env->skip_count);
            }
            if (env->pass_count > 0) {
                pizza_io_printf("::group::{bold}Test Success Analysis (CI Mode){reset}\n");
                pizza_io_printf("SUCCESS_RATE=%.2f%%\n", success_rate);
            }

            pizza_io_printf("SUCCESS_PROBABILITY=%.2f\n", probability_of_success);
            pizza_io_printf("AVERAGE_TEST_RATE=%.2f%%\n", average_rate);
            pizza_io_printf("PREDICTION=%.2f%%\n", prediction);
            pizza_io_printf("::endgroup::\n");

            if (env->skip_count > 0) {
                pizza_io_printf("::group::{bold}Skipped Tests Analysis (CI Mode){reset}\n");
                pizza_io_printf("SKIPPED_TESTS=There were %d skipped tests. Please verify conditions or requirements for those tests.\n", env->skip_count);
                pizza_io_printf("::endgroup::\n");
            }
            break;
        ...
        case FOSSIL_TEST_SUMMARY_JELLYFISH:
            ...
            if (env->fail_count > 0) {
                pizza_io_printf("Failure Rate: %.2f%%\n", failure_rate);
                pizza_io_printf("Consider reviewing the failed tests for possible issues with the environment, misconfigurations, or test logic.\n");
            }
            if (env->timeout_count > 0) {
                pizza_io_printf("Timeout Rate: %.2f%%\n", timeout_rate);
                pizza_io_printf("Investigate system resources, network conditions, or tests prone to timeouts.\n");
            }
            if (env->skip_count > 0) {
                pizza_io_printf("Skipped Tests: %.2f%% (%d tests)\n", skip_rate, env->skip_count);
                pizza_io_printf("Ensure tests are not being skipped due to unmet conditions. Review the prerequisites or intentional exclusions.\n");
            }
            if (env->pass_count > 0) {
                pizza_io_printf("Success Rate: %.2f%%\n", success_rate);
                pizza_io_printf("Great job! Consider reviewing the successful tests for potential optimizations or edge cases.\n");
            }

            pizza_io_printf("Probability of Success: %.2f\n", probability_of_success);
            pizza_io_printf("Average Test Rate: %.2f%%\n", average_rate);
            pizza_io_printf("Prediction (Future Success Rate): %.2f%%\n", prediction);
    ...
    switch (env->options.summary) {
        ...
        case FOSSIL_TEST_SUMMARY_JELLYFISH:
            ...
            if (env->pass_count == 0 && env->fail_count == 0 && env->skip_count == 0 && env->timeout_count == 0 && env->empty_count > 0) {
                pizza_io_printf("{cyan}{italic}Suggestion: %s{reset}\n", empty_suite_suggestions[rand() % 50]);
            } else if (env->fail_count > 0) {
                ...
                pizza_io_printf("{cyan}{italic}Suggestion: Review skipped tests for prerequisites or intentional exclusions. Ensure tests are not being skipped due to unmet conditions.{reset}\n");
            }
            break;
        ...
        case FOSSIL_TEST_SUMMARY_CI:
            ...
            if (env->fail_count > 0) {
                pizza_io_printf("::group::{bold}Test Failure Suggestion (CI Mode){reset}\n");
                pizza_io_printf("FAILURE_SUGGESTION=%s\n", failure_suggestions[rand() % 50]);
                pizza_io_printf("::endgroup::\n");
            } else if (env->pass_count > 0) {
                pizza_io_printf("::group::{bold}Test Success Suggestion (CI Mode){reset}\n");
                pizza_io_printf("SUCCESS_SUGGESTION=%s\n", success_suggestions[rand() % 50]);
                pizza_io_printf("::endgroup::\n");
            } else if (env->timeout_count > 0) {
                pizza_io_printf("::group::{bold}Test Timeout Suggestion (CI Mode){reset}\n");
                pizza_io_printf("TIMEOUT_SUGGESTION=%s\n", timeout_suggestions[rand() % 50]);
                pizza_io_printf("::endgroup::\n");
            } else if (env->skip_count > 0) {
                pizza_io_printf("::group::{bold}Test Skipped Suggestion (CI Mode){reset}\n");
                pizza_io_printf("SKIPPED_SUGGESTION=Review skipped tests for prerequisites or intentional exclusions.\n");
                pizza_io_printf("Ensure tests are not being skipped due to unmet conditions.\n");
                pizza_io_printf("::endgroup::\n");
            }
            break;
...
void fossil_test_execution_time(fossil_test_env_t *env) {
    ...
    switch (env->options.summary) {
        ...
        case FOSSIL_TEST_SUMMARY_JELLYFISH:
            ...
            pizza_io_printf("{cyan}{italic}\tExecution Time Analysis (Plain Mode):{reset}\n");
            pizza_io_printf("{cyan}{italic}|\tExecution time: %02d sec, %03d ms, %06d us, %09d ns{reset}\n",
        ...
        case FOSSIL_TEST_SUMMARY_CI:
            ...
            pizza_io_printf("::group::{bold}Execution Time (CI Mode){reset}\n");
            pizza_io_printf("EXECUTION_TIME=%02d sec %03d ms %06d us %09d ns\n", 
                                  seconds, milliseconds, microseconds, nanoseconds);
            pizza_io_printf("::endgroup::\n");
            break;
...
void fossil_test_summary(fossil_test_env_t *env) {
    ...
    if (env->options.dry_run) {
        pizza_io_printf("{purple}Dry run mode enabled. No tests were executed or evaluated.{reset}\n");
        return;
    }
        ...
        while (test != NULL) {
            if (test->status == TEST_STATUS_PASS) {
                ...
                if (test->failure_message && env->options.summary != FOSSIL_TEST_SUMMARY_CI) {
                    pizza_io_printf("{red}Test '%s' failed: %s{reset}\n", test->name, test->failure_message);
                }
    ...
    pizza_io_printf("{blue}{bold}=================================================================================={reset}\n");
    pizza_io_printf("{cyan}{italic}\tFossil Smart Test                                                             {reset}\n");
    pizza_io_printf("{blue}{bold}=================================================================================={reset}\n");

    switch (env->options.summary) {
        case FOSSIL_TEST_SUMMARY_PLAIN:
            pizza_io_printf("{blue}{bold}Fossil Test Summary:{reset}\n");
            pizza_io_printf("{cyan}{italic}Passed:   {green}%d{reset}\n", env->pass_count);
            pizza_io_printf("{cyan}{italic}Failed:   {red}%d{reset}\n", env->fail_count);
            pizza_io_printf("{cyan}{italic}Skipped:  {yellow}%d{reset}\n", env->skip_count);
            pizza_io_printf("{cyan}{italic}Timeouts: {magenta}%d{reset}\n", env->timeout_count);
            pizza_io_printf("{blue}{bold}Other:    {blue}%d{reset}\n", env->unexpected_count);
        ...
        case FOSSIL_TEST_SUMMARY_CI:
            pizza_io_printf("::group::{bold}Fossil Test Summary{reset}\n");
            pizza_io_printf("PASS=%d\n", env->pass_count);
            pizza_io_printf("FAIL=%d\n", env->fail_count);
            pizza_io_printf("SKIP=%d\n", env->skip_count);
            pizza_io_printf("TIMEOUT=%d\n", env->timeout_count);
            pizza_io_printf("OTHER=%d\n", env->unexpected_count);
            pizza_io_printf("::endgroup::\n");
            fossil_test_comment(env);   // AI-style comments
            fossil_test_suggest(env);   // Suggestions for test coverage or structure
            fossil_test_execution_time(env);
